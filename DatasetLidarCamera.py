...
class DatasetLidarCameraKittiOdometry(Dataset):
    ...
    def __init__(self, dataset_dir, transform=None, augmentation=False, use_reflectance=False, blur_degree=0, snr=1.0,
                 guass_kernel=0, max_t=1.5, max_r=20., split='val', device='cpu', val_sequence='00', suf='.png'):
        super(DatasetLidarCameraKittiOdometry, self).__init__()
        self.max_points = 150000
        self.use_reflectance = use_reflectance
        self.blur_degree = blur_degree
        self.snr = snr
        self.guass_kernel = guass_kernel
        self.maps_folder = ''
        self.device = device
        self.max_r = max_r
        self.max_t = max_t
        ...
        self.all_files = []
        self.sequence_list = ['00', '01', '02', '03', '04', '05', '06', '07', '08', '09', '10', '11', '12', '13',
    ...
    def __getitem__(self, idx):
        ...
        valid_indices = valid_indices | (pc[:, 1] > 3.)
        pc = pc[valid_indices].copy()  # [N, 4]
        # pad or cut
        format_data = np.zeros([120000, 4], np.float32)
        if len(pc) < 120000:
            pad_data = pc[-1]
            format_data[0:len(pc)] = pc
            format_data[len(pc):] = pad_data
        else:
            format_data = pc[0:120000]

        pc_org = torch.from_numpy(format_data.astype(np.float32))
        ...
        if self.blur_degree > 0:
            ...
            img_blur = motion_blur(img_array, degree=self.blur_degree)
            img = Image.fromarray(np.uint8(img_blur))
        if self.guass_kernel > 0:
            img_array = np.array(img)
            img_guass = cv2.GaussianBlur(img_array, ksize=(self.guass_kernel, self.guass_kernel), sigmaX=0, sigmaY=0)
            img = Image.fromarray(np.uint8(img_guass))
        if self.snr < 1.0:
            img_array = np.array(img)
            img_sp = addsalt_pepper(img_array, self.snr)
            img = Image.fromarray(np.uint8(img_sp))
        img_rotation = 0.
        # if self.split == 'train':
        ...
        if self.split == 'test':
            sample = {'rgb': img, 'point_cloud': pc_in, 'calib': calib, 'tr_error': T, 'rot_error': R, 'seq': int(seq),
                      'img_path': img_path, 'rgb_name': rgb_name + '.png', 'item': item, 'extrin': RT,
...
if __name__ == '__main__':
    data_dir = '/data1/zhuangfan/kitti/odometry/dataset'
    dataset = DatasetLidarCameraKittiOdometry(dataset_dir=data_dir, split='train', blur_degree=10)
    ...
    for i, s in enumerate(dataset):
        if i%50 == 0:
            img = s['rgb']
            pc_cam = s['point_cloud']
            K = s['calib']
            T = s['extrin']
            _, h, w = img.shape
            depth, _ = lidar_project_depth(pc_cam, K, [h, w])
            # inpaint
            img = img.unsqueeze(0).cpu()
            depth_img = depth.unsqueeze(0).cpu()/80.0

            depth_img = F.max_pool2d(depth_img, kernel_size=5, stride=1, padding=2)
            show_tensor(depth_img)
