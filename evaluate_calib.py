...
def config():
    ...
    repeat = 3
    sample_num = 4096
    weights = 'checkpoints/kitti/odom0/val_seq_00/model/checkpoint_r20.00_t1.50_e93_0.010.tar'
    output = 'output_snr_0.85'
    save_image = False
...
def main(_config, seed):
    ...
    if _config['test_sequence'] is None:
        ...
        dataset_val = dataset_class(_config['data_folder'], max_r=_config['max_r'], max_t=_config['max_t'],
                                    split='test', use_reflectance=_config['use_reflectance'], snr=0.85,
    ...
    lcc = LCANet(img_shape=img_shape, depth_max=_config['max_depth'], sample_num=_config['sample_num'], repeat=_config['repeat'])
    checkpoint = torch.load(_config['weights'], map_location='cpu')
    saved_state_dict = checkpoint['state_dict']
    lcc.load_state_dict(saved_state_dict)
    lcc = lcc.to(device)
    lcc.eval()
    ...
    _config['output'] = os.path.join(_config['output'], 'repeat'+str(_config['repeat']))
    ...
    depth_pred_path = os.path.join(_config['output'], 'depth', 'pred')
    ...
    for batch_idx, sample in enumerate(tqdm(TestImgLoader)):
        ...
        if _config['save_image']:
            ...
            depth_img = lidar_input.detach().cpu().numpy()
            depth_img = depth_img * 255
            cv2.imwrite(os.path.join(depth_path, sample['rgb_name'][0]), depth_img[0, 0, :376, :1241])
        ...
        pc_rotated_input = pc_rotated_input.to(device)
        ...
        with torch.no_grad():
            t1 = time.time()
            pre_transls, pre_rots = lcc(rgb_resize, pc_rotated_input, sample['calib'])
            T_predicted, R_predicted = pre_transls[-1], pre_rots[-1]
            run_time = time.time() - t1
            if _config['rot_transl_separated']:
            ...
            RTs.append(torch.mm(RTs[0], RT_predicted)) # inv(H_gt)*H_pred_1*H_pred_2*.....H_pred_n
           
            rotated_point_cloud = rotate_forward(pc_rotated_input[0], RT_predicted) # H_pred*X_init

            depth_pred, uv_pred, pc_pred_valid = lidar_project_depth(rotated_point_cloud, sample['calib'][0],
                                                                        real_shape_input[0])  # image_shape
            lidar = depth_pred.unsqueeze(0)
            lidar = F.max_pool2d(lidar, kernel_size=5, stride=1, padding=2)
            lidar = F.pad(lidar, shape_pad_input[0])
            ...
            if _config['save_image']:
                ...
                lidar = (lidar / _config['max_depth']) * 255
                depth_pred = lidar[0, 0].detach().cpu().numpy()
                cv2.imwrite(depth_pred_path + '/{}.png'.format(batch_idx), depth_pred)


            if _config['save_image']:
                out2 = overlay_imgs(rgb_input[0], lidar)
                out2 = out2[:376, :1241, :]
                cv2.imwrite(os.path.join(os.path.join(pred_path), sample['rgb_name'][0]), out2[:, :, [2, 1, 0]]*255)

                out2 = overlay_imgs(rgb_input[0], F.pad(inte_lidar, shape_pad_input[0])/_config['max_depth'])
                out2 = out2[:376, :1241, :]
                cv2.imwrite(os.path.join(os.path.join(inte_pred_path), sample['rgb_name'][0]), out2[:, :, [2, 1, 0]] * 255)

            if show:
                out2 = overlay_imgs(rgb_input[0], lidar)
                cv2.imshow(f'Pred', out2[:, :, [2, 1, 0]])
                cv2.waitKey(1)

            # inv(H_init)*H_pred
            T_composed = RTs[1][:3, 3]
            R_composed = quaternion_from_matrix(RTs[1])
            errors_t[1].append(T_composed.norm().item())
            errors_t2[1].append(T_composed)
            errors_r[1].append(quaternion_distance(R_composed.unsqueeze(0),
                                                                torch.tensor([1., 0., 0., 0.], device=R_composed.device).unsqueeze(0),
                                                                R_composed.device))

            # rpy_error = quaternion_to_tait_bryan(R_composed)
            rpy_error = mat2xyzrpy(RTs[1])[3:]
            rpy_error *= (180.0 / 3.141592)
            errors_rpy[1].append(rpy_error)
            log_string += [str(errors_t[1][-1]), str(errors_r[1][-1]),
                            str(errors_t2[1][-1][0].item()), str(errors_t2[1][-1][1].item()),
                            str(errors_t2[1][-1][2].item()), str(errors_rpy[1][-1][0].item()),
                            str(errors_rpy[1][-1][1].item()), str(errors_rpy[1][-1][2].item())]
    ...
    for i in range(2):
        ...
        with open(os.path.join(_config['output'], 'results.txt'), 'a', encoding='utf-8') as f:
    ...
    print("average runing time on {} iteration: {} s".format(1, avg_time))
    print("End!")
